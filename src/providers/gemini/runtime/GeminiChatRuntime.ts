...
import { updateGeminiDiscoveryState } from '../discoveryState';
import {
...
import {
  combineGeminiRawModelSelection,
  decodeGeminiModelId,
  encodeGeminiModelId,
...
export class GeminiChatRuntime implements ChatRuntime {
  ...
  async ensureReady(options?: ChatRuntimeEnsureReadyOptions): Promise<boolean> {
    ...
    const nextLaunchKey = JSON.stringify({
      ...
      envText: getRuntimeEnvironmentText(this.plugin.settings as unknown as Record<string, unknown>, 'gemini'),
    });
  ...
  buildSessionUpdates(params: {
    ...
    const existingState = params.conversation
      ? getGeminiState(params.conversation.providerState)
      : null;
  ...
  private resolveSelectedRawModelId(queryOptions?: ChatRuntimeQueryOptions): string | null {
    ...
    if (!isGeminiModelSelectionId(selectedModel)) {
      return null;
    }

    const selectedBaseRawModelId = decodeGeminiModelId(selectedModel);
    if (!selectedBaseRawModelId) {
      return null;
    }

    const discoveredModels = getGeminiProviderSettings(providerSettings).discoveredModels;
    const effortLevel = typeof providerSettings.effortLevel === 'string'
      ? providerSettings.effortLevel
      : GEMINI_DEFAULT_THINKING_LEVEL;
    const normalizedBaseRawModelId = resolveGeminiBaseModelRawId(selectedBaseRawModelId, discoveredModels);
    const resolvedRawModelId = combineGeminiRawModelSelection(
  ...
  private getActiveDisplayModel(queryOptions?: ChatRuntimeQueryOptions): string | undefined {
    ...
    if (
      selectedModel
      && selectedModel !== GEMINI_SYNTHETIC_MODEL_ID
      && isGeminiModelSelectionId(selectedModel)
    ) {
      const selectedRawModelId = this.resolveSelectedRawModelId(queryOptions);
      return selectedRawModelId
        ? encodeGeminiModelId(selectedRawModelId)
        : selectedModel;
    }

    return this.currentSessionModelId
      ? encodeGeminiModelId(this.currentSessionModelId)
      : (selectedModel && isGeminiModelSelectionId(selectedModel) ? selectedModel : undefined);
  }

  private resolveSelectedModeId(): string | null {
    const providerSettings = this.getProviderSettings();
    const geminiSettings = getGeminiProviderSettings(providerSettings);
    const availableModes = getManagedGeminiModes(geminiSettings.availableModes);
    const mappedModeId = resolveGeminiModeForPermissionMode(
      providerSettings.permissionMode,
      geminiSettings.availableModes,
    );
    if (mappedModeId) {
      return mappedModeId;
    }

    if (geminiSettings.selectedMode) {
      if (
        availableModes.some((mode) => mode.id === geminiSettings.selectedMode)
      ) {
        return geminiSettings.selectedMode;
      }
    }
  ...
  private async syncSessionModelState(params: {
    ...
    const discoveredModels = normalizeGeminiDiscoveredModels(
    ...
    const currentSettings = getGeminiProviderSettings(settingsBag);
    const currentBaseRawModelId = currentRawModelId
      ? resolveGeminiBaseModelRawId(currentRawModelId, discoveredModels)
      : null;
    const currentThinkingLevel = currentRawModelId
      ? extractGeminiModelVariantValue(currentRawModelId, discoveredModels)
      : null;
    ...
    const discoveryChanged = discoveredModels.length > 0
      ...
      && updateGeminiDiscoveryState(settingsBag, { discoveredModels });
    ...
    if (currentBaseRawModelId) {
      const seeded = this.seedActiveModelSelection(
        settingsBag,
        encodeGeminiModelId(currentBaseRawModelId),
  ...
  private seedActiveModelSelection(
    ...
    const savedModel = typeof savedProviderModel.gemini === 'string'
      ? savedProviderModel.gemini
      : '';
    if (!savedModel || savedModel === GEMINI_SYNTHETIC_MODEL_ID) {
      savedProviderModel.gemini = modelSelection;
      changed = true;
    }

    if (thinkingLevel) {
      const savedProviderEffort = ensureProviderProjectionMap(settingsBag, 'savedProviderEffort');
      if (typeof savedProviderEffort.gemini !== 'string' || !savedProviderEffort.gemini) {
        savedProviderEffort.gemini = thinkingLevel;
        changed = true;
      }
    }
    ...
    if (!activeModel || activeModel === GEMINI_SYNTHETIC_MODEL_ID) {
  ...
  private async syncSessionModeState(params: {
    ...
    const availableModes = normalizeGeminiAvailableModes(acpState.availableModes);
    ...
    if (shouldSeedSelectedMode && currentModeId) {
      updateGeminiProviderSettings(settingsBag, { selectedMode: currentModeId });
      await this.plugin.saveSettings();
    }
    this.refreshModelSelectors();
  }
  ...
  private emitPermissionModeSync(modeId: string): void {
    const permissionMode = resolvePermissionModeForManagedGeminiMode(modeId);
  ...
  private async handlePermissionRequest(
    ...
    const presentation = buildGeminiPermissionPresentation(request.toolCall.title, input, request.toolCall.locations);
...
function buildGeminiPermissionPresentation(
  ...
  switch (permissionId) {
    case 'bash':
      return {
        ...
        description: 'Gemini wants to run a shell command.',
        toolName: 'bash',
      };
    case 'codesearch':
      return {
        description: 'Gemini wants to search indexed code outside the active buffer.',
        toolName: 'codesearch',
      };
    case 'doom_loop': {
      const repeatedTool = typeof input.tool === 'string' ? input.tool.trim() : '';
      return {
        decisionReason: 'Gemini detected repeated identical tool calls',
    ...
    case 'edit':
      return {
        ...
        description: blockedPath
          ? 'Gemini wants to modify this file.'
          : 'Gemini wants to apply file changes.',
        toolName: 'edit',
      };
    case 'external_directory':
      return {
        ...
        description: blockedPath
          ? 'Gemini wants to access a path outside the working directory.'
          : 'Gemini wants to access files outside the working directory.',
        toolName: 'External Directory',
      };
    case 'glob':
      return {
        description: 'Gemini wants to scan file paths with a glob pattern.',
        toolName: 'glob',
      };
    case 'grep':
      return {
        description: 'Gemini wants to search file contents with a pattern.',
        toolName: 'grep',
      };
    case 'lsp':
      return {
        description: 'Gemini wants to query language server data.',
        toolName: 'lsp',
      };
    case 'plan_enter':
      return {
        description: 'Gemini wants to switch this session into planning mode.',
        toolName: 'Enter Plan Mode',
      };
    case 'plan_exit':
      return {
        description: 'Gemini wants to leave planning mode and resume implementation.',
        toolName: 'Exit Plan Mode',
      };
    case 'question':
      return {
        description: 'Gemini wants to ask you a direct question before continuing.',
    ...
    case 'read':
      return {
        ...(blockedPath ? { blockedPath } : {}),
        description: blockedPath
          ? 'Gemini wants to read this path.'
          : 'Gemini wants to read project files.',
        toolName: 'read',
      };
    case 'skill':
      return {
        description: 'Gemini wants to load a skill into the current session.',
        toolName: 'skill',
      };
    case 'todowrite':
      return {
        description: 'Gemini wants to update the shared task list.',
        toolName: 'todowrite',
      };
    case 'webfetch':
      return {
        description: 'Gemini wants to fetch content from a URL.',
        toolName: 'webfetch',
      };
    case 'websearch':
      return {
        description: 'Gemini wants to search the web.',
        toolName: 'websearch',
      };
    ...
    default:
      return {
        ...(blockedPath ? { blockedPath } : {}),
        description: blockedPath
          ? `Gemini wants permission to use ${formatPermissionLabel(permissionId)} on this path.`
          : `Gemini wants permission to use ${formatPermissionLabel(permissionId)}.`,
